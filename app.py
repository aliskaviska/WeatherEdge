...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    with st.container(border=True):
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.metric("NWS high", "—" if pd.isna(r["nws_high_f"]) else f"{int(r['nws_high_f'])}°F")
...
with st.expander("See rejected / conflicting contracts"):
    ...
    if rejected.empty:
        ...
        rejected["NWS"] = rejected["nws_high_f"].map(lambda x: "—" if pd.isna(x) else f"{int(x)}°F")
        ...
        st.dataframe(
            rejected[
                ["city", "date_label", "side", "market_subtitle", "Status", "NWS", "Ensemble", "Price", "Gap"]
            ].rename(columns={
                "city": "City",
                "date_label": "Date",
                "side": "Side",
...
with st.expander("How to read this"):
    st.markdown(
        """
**NWS high** is the official point forecast.

**Ensemble median** is the middle forecast across the GFS ensemble members.
