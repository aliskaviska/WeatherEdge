...
def build_city_rows(city, cfg):
    ...
    for m in markets:
        ...
        p_yes, n = probability(ens.loc[d].values, kind, lo, hi)
        if p_yes is None:
            continue
...
st.caption("Live Kalshi weather prices + forecast probabilities + direct links to the exact dated Kalshi events. Research tool, not a profit guarantee.")
...
for rank, (_, r) in enumerate(qualified.iterrows(), start=1):
    ...
    with st.container(border=True):
        ...
        st.markdown(f"## **BET {r['side']} on {r['bracket']}**")
        st.write(f"**Kalshi event:** {r['event_title']}")
        st.write(f"**Exact market label:** {r['market_subtitle']}")
        st.write(f"**How to identify it on Kalshi:** search for the event above, then choose the **{r['bracket']}** temperature outcome and buy **{r['side']}**.")
        st.code(f"Market ticker: {r['market_ticker']}\nEvent ticker: {r['event_ticker']}\nSeries: {r['series_ticker']}", language=None)

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Price to buy", f"{r['ask']*100:.0f}¢")
        c2.metric("Model chance", fmt_pct(r["model_prob"]))
        c3.metric("Conservative chance", fmt_pct(r["conservative_prob"]))
        c4.metric("Conservative edge", f"{r['conservative_edge']*100:+.1f} pp")

        if r["ask"] > 0:
            contracts_for_10 = int(10 // r["ask"])
            if contracts_for_10 > 0:
                cost = contracts_for_10 * r["ask"]
                gross = contracts_for_10
                profit = gross - cost
                st.write(
                    f"**Simple payout example:** {contracts_for_10} contracts would cost about **${cost:.2f}** "
                    f"and pay **${gross:.2f} total** if they settle in your favor, for about **${profit:.2f} gross profit before fees**."
                )

        nws_bits = []
        if pd.notna(r["nws_high_f"]):
            nws_bits.append(f"NWS forecast high: **{int(r['nws_high_f'])}°F**")
        if r["nws_forecast"]:
            nws_bits.append(str(r["nws_forecast"]))
        if nws_bits:
            st.write(" · ".join(nws_bits))

        st.write(f"**Settlement source shown by Kalshi API:** {r['settlement_source']}")
        st.write(f"**Station/location cross-check:** {r['station_hint']}")

        if r["suspicious"]:
            st.warning(
                "This edge is unusually large. Do not treat it as an automatic bet. "
                "Open the Kalshi contract and verify the exact settlement station, date, and temperature bracket first."
            )
        ...
        st.caption(
            f"Direct dated event: {r['event_ticker']}. Once it opens, select the "
            f"**{r['bracket']}** row and choose **{r['side']}**."
        )
        if r["contract_url"]:
            st.link_button(
                "View Kalshi contract/rules source ↗",
                r["contract_url"],
                use_container_width=True,
            )
...
table["Model"] = table["model_prob"].map(fmt_pct)
table["Conservative"] = table["conservative_prob"].map(fmt_pct)
table["Cons. edge"] = table["conservative_edge"].map(lambda x: f"{x*100:+.1f} pp")
table["NWS"] = table["nws_high_f"].map(lambda x: "—" if pd.isna(x) else f"{int(x)}°F")
table["Flag"] = table.apply(lambda r: classify(r["conservative_edge"], r["suspicious"]), axis=1)

display_cols = [
    "Rank", "Flag", "city", "date_label", "Bet", "Price",
    "Model", "Conservative", "Cons. edge", "NWS",
    "market_ticker", "event_ticker", "kalshi_event_url"
]
st.dataframe(
...
st.subheader("How to use this without getting fooled")
st.markdown(
    """
1. Start with the **top cards**, not the giant table.
2. Read the card literally: **city + date + BET YES/NO + exact temperature bracket**.
3. In Kalshi, find the matching event and bracket. Confirm the **market ticker** if you are unsure.
4. Check the settlement source and station. If they do not match what you expect, **pass**.
5. Favor positive **Conservative edge**, not just raw Model chance.
6. A giant 30–50 point edge gets a **CHECK** flag because data mismatches can masquerade as amazing bets.
7. Start small until you have enough completed trades to see whether the model is actually calibrated.
"""
)

st.info(
    "WeatherEdge estimates probabilities from GFS ensemble members. It does not yet correct for station-specific model bias, "
    "Kalshi fees, bid/ask depth, or historical forecast accuracy. Those are the next upgrades worth making."
)
